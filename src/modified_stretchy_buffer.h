...
#include "physfs.h" // use custom PhysFS allocators instead of C stdlib
...
#define stb_sb_free(a)         ((a) ? allocator.Free(stb__sbraw(a)),0 : 0)
...
static void * stb__sbgrowf(void *arr, int increment, int itemsize)
{
   ...
   int *p = (int *) allocator.Realloc(arr ? stb__sbraw(arr) : 0, itemsize * m + sizeof(int)*2);
   if (p) {
      if (!arr)
         p[1] = 0;
      p[0] = m;
